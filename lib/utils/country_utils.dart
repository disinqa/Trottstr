...
class CountryUtils {
  ...
  /// Generate all countries dynamically from comprehensive flag data
  static List<CountryInfo> get _countries {
    return CountryFlags.countryFlags.entries
        .where((entry) => entry.key.length == 2) // Only valid country codes
        .map(
          (entry) => CountryInfo(
            code: entry.key,
            flag: entry.value,
            name: _countryNames[entry.key] ?? entry.key,
          ),
        )
        .toList();
  }
